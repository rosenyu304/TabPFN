...
def setup_regressor(config: dict) -> tuple[TabPFNRegressor, dict]:
    ...
    regressor_config = {
        ...
        "inference_config": {
            "REGRESSION_Y_PREPROCESS_TRANSFORMS": (None, None) # Force target transform to None
        },
    }
...
def main(id: int = 0, 
            ...
            for epoch in range(config["finetuning"]["epochs"] + 1):
                ...
                if epoch > 0:
                    val_mse, val_mae, val_r2, val_rmse = evaluate_regressor(
                        regressor, eval_config, X_train, y_train, batch_x_raw[0,:,:], batch_y_test_raw[0,:]
                    )
    ...
    os.makedirs(f'finetune_0814_openml', exist_ok=True)

    RMSE_DF = pd.DataFrame(RMSE_LIST)
    RMSE_DF.to_csv(f"finetune_0814_openml/RMSE_LIST_openml_{id}_{np.log10(LAMBDA)}.csv", index=False)



if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", type=int, default=44978)
